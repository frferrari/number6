...
class BatchEntitySpec extends ScalaTestWithActorTestKit(EventSourcedBehaviorTestKit.config.withFallback(BatchEntityTestConfig.config))
  ...
  "Batch" should {
    "SUCCEED to Create when in [EmptyBatch] state" in {
      ...
      val result = eventSourcedTestKit.runCommand[StatusReply[Done]](Create(batchID, batchSpecification1, Nil, _))
      result.reply shouldBe StatusReply.Ack
      result.eventOfType[Created].batchID shouldBe batchID
      result.eventOfType[Created].batchSpecification.batchSpecificationID shouldBe batchSpecification1.batchSpecificationID
    ...
    "SUCCEED to MatchAuction when in [ActiveBatch] state" in {
      ...
      eventSourcedTestKit.runCommand[StatusReply[Done]](Create(batchID, batchSpecification1, auctions, _))
    ...
    "SUCCEED to Stop when in [ActiveBatch] state" in {
      val createResult = eventSourcedTestKit.runCommand[StatusReply[Done]](Create(UUID.randomUUID(), batchSpecification1, Nil, _))
