...
import com.fferrari.model.{Batch, WebsiteConfig}
  ...
  private def processListingPage[V <: AuctionValidator](validator: V,
    ...
    Behaviors.withTimers[AuctionScraperCommand] { implicit timers =>
      Behaviors.receivePartial {
        ...
        case (context, ExtractAuctions(batchSpecification, batch@Batch(batchId, _, batchAuctionLinks), pageNumber)) =>
                ...
                case Valid(auction) =>
                  ...
                  timers.startSingleTimer(
                    ExtractAuctions(
                      ...
                      batch.copy(auctionUrls = remainingAuctionUrls),
                      pageNumber + 1
                    ), randomDurationMs())
                  Behaviors.same
